...
public:
	unsigned int ID;
	int i = 0;
	Lighting(const char* vertexPath, const char* fragmentPath);
	Lighting(const char* vertexPath, const char* fragmentPath, int numOfPointLights);
	void use();
	...
	void pointLight(int ID,glm::vec3 position, glm::vec3 ambient, glm::vec3 diffuse, glm::vec3 specular, float constant, float linear, float quadratic);
