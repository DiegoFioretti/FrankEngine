...
Shader3D::Shader3D(const char* vertexPath, const char* fragmentPath, const char* geometryPath)
{
	...
	try
	{
		...
		//fragmentCode.replace(fragmentCode.begin(),fragmentCode.end(),"NR_POINT_LIGHTS 1","NR_POINT_LIGHTS 2");
...
Shader3D::Shader3D(const char* vertexPath, const char* fragmentPath, int numOfPointLights,const char* geometryPath)
{
	...
	try
	{
		...
		string a = "NR_POINT_LIGHTS ";
		stringstream b;
		b << numOfPointLights;
		a.append(b.str());
		fragmentCode = regex_replace(fragmentCode,regex("NR_POINT_LIGHTS 1"), a);
...
void Shader3D::checkCompileErrors(GLuint shader, std::string type)
{
	GLint success;
	GLchar infoLog[1024];
	if (type != "PROGRAM")
